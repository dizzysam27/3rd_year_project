...
class MazeSolver:
    ...
    # --- Find Nearest Valid Point for A*
    ...
    def run(self):
        ...
        while not self.path_found_and_generated:
            ...
            if valid_start and valid_end:
                ...
                cv2.imshow("DEBUG A* Start-End on Obstacles", debug_mask)
                 ...
                 try: cv2.destroyWindow("DEBUG A* Start-End on Obstacles")
                 except: pass
        ...
        try: cv2.destroyWindow("Maze Solver - Pathfinding")
        except: pass
        ...
        if not self.path_found_and_generated or self.final_frame is None:
            ...
            while True:
                ...
                if vis_green_mask is not None: output_image[vis_green_mask == 255] = (0, 255, 0) # Green walls
                ...
                start_r = max(0, min(start_point_yx[0], h - 1)); start_c = max(0, min(start_point_yx[1], w - 1))
                end_r = max(0, min(end_point_yx[0], h - 1)); end_c = max(0, min(end_point_yx[1], w - 1))
                ...
                cv2.putText(output_image, "Path Calculated - Press 'q' to exit", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 3, cv2.LINE_AA);
                cv2.putText(output_image, "Path Calculated - Press 'q' to exit", (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
