...
def simplify_path_by_sampling(path_points, num_points):
    ...
    if not path_points or num_points <= 0:
        return []
    path_len = len(path_points)
    if path_len <= num_points:
        return path_points
...
class MazeSolver:
    def __init__(self):
        ...
        # --- Pathfinding State ---
        self.stored_path_yx = None
        self.waypoints_yx = None  # Waypoints added
    ...
    def crop_frame(self, frame):
        ...
        end_y_clamped = min(frame_height, end_y)
        start_x_clamped = max(0, start_x)
        end_x_clamped = min(frame_width, end_x)
        if start_y_clamped >= end_y_clamped or start_x_clamped >= end_x_clamped:
            print(
                f"Warning: Invalid crop {crop_width}x{crop_height}. Using full frame {frame_width}x{frame_height}.")
            return None
    ...
    def detect_features(self, frame):
        ...
        kernel = self.detection_kernel
        _, dark_regions_mask = cv2.threshold(gray, self.hole_darkness_threshold, 255, cv2.THRESH_BINARY_INV)
        hole_mask_cleaned = cv2.morphologyEx(dark_regions_mask, cv2.MORPH_OPEN, kernel)
        hole_mask_cleaned = cv2.morphologyEx(hole_mask_cleaned, cv2.MORPH_CLOSE, kernel, iterations=1)
        green_mask = cv2.inRange(hsv, self.lower_green, self.upper_green)
        green_mask_cleaned = cv2.morphologyEx(green_mask, cv2.MORPH_CLOSE, kernel, iterations=1)
        green_mask_cleaned = cv2.morphologyEx(green_mask_cleaned, cv2.MORPH_OPEN, kernel, iterations=1)
        not_wall_mask = cv2.bitwise_not(green_mask_cleaned)
        final_hole_mask = cv2.bitwise_and(hole_mask_cleaned, not_wall_mask)
        return green_mask_cleaned, final_hole_mask

    def find_nearest_valid_point(self, point_yx, validation_obstacle_mask):
        r, c = point_yx
        h, w = validation_obstacle_mask.shape
        if not (0 <= r < h and 0 <= c < w):
            return None
        if validation_obstacle_mask[r, c] == 0:
            return point_yx
        ...
        for radius_search in range(1, max_search_radius + 1):
            for dr in range(-radius_search, radius_search + 1):
                for dc in range(-radius_search, radius_search + 1):
                    if abs(dr) != radius_search and abs(dc) != radius_search:
                        continue
                    nr, nc = r + dr, c + dc
                    if not (0 <= nr < h and 0 <= nc < w):
                        continue
                    if validation_obstacle_mask[nr, nc] == 0:
                        return (nr, nc)
        print(f"Could not find valid point near {point_yx}.")
        return None
    ...
    def run(self):
        ...
        while not self.path_found_and_generated:
            ...
            start_r = max(0, min(start_point_yx[0], h_crop - 1))
            start_c = max(0, min(start_point_yx[1], w_crop - 1))
            end_r = max(0, min(end_point_yx[0], h_crop - 1))
            end_c = max(0, min(end_point_yx[1], w_crop - 1))
            ...
            # Debug visualization of A* start/end
            if valid_start and valid_end:
                debug_mask = cv2.cvtColor(validation_obstacle_mask, cv2.COLOR_GRAY2BGR)
                cv2.circle(debug_mask, (valid_start[1], valid_start[0]), 5, (0, 255, 0), -1)  # Green start
                cv2.circle(debug_mask, (valid_end[1], valid_end[0]), 5, (0, 0, 255), -1)  # Red end
                cv2.imshow("DEBUG A* Start-End on Obstacles", debug_mask)
            else:
                try:
...
def astar_repulsive(start, end, grid_shape, green_wall_mask, dist_from_holes, repulsion_strength,
                    repulsion_threshold_norm):
    # (A* logic unchanged)
    def heuristic(a, b):
