...
class ImageProcessor:
    ...
    def run(self, waypoints):
        ...
        for goal in waypoints:
            goal1 = goal[1]
            waypoints[index][1] = goal[0]
            waypoints[index][0] = goal1
            index += 1
        ...
        while True:
            ...
            cropped_frame = self.rotate_frame(cropped_frame)
            for goal in waypoints:
                cv2.circle(cropped_frame, (goal[0], goal[1]), 3, (0, 150, 255), -1)
...
try:
    ...
    a_star.cleanup()
    index = 0
    for point in a_star_waypoints:
       
       a_star_waypoints[index][0] = (point[0] * 1.16) + 10
       a_star_waypoints[index][1] = (point[1] * 1.12) - 10
    
       index += 1
    a_star_waypoints = a_star_waypoints[1:]
